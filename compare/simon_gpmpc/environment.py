...
class e_trajectory_simENV(gym.Env, ABC):
    ...
    def __init__(self, **kwargs):
        ...
        self.observation_space = spaces.Box(
            low=np.append(low, 0), high=np.append(high, 1), dtype=np.float32
        )
        ...
        self.state_scale = (
            100  # Meters to cms as given from BPMs in the measurement later on
        )
    ...
    def reset(self, **kwargs):
        ...
        while bad_init:
            ...
            bad_init = (
                any(abs(return_initial_state) > 10 * abs(self.threshold))
    ...
    def step(self, action, reference_position=None):
        ...
        if (abs(return_state) >= abs(10 * self.threshold)).any():
            ...
            first_hit_position = np.argmax(
                abs(return_state) >= abs(10 * self.threshold)
            )
            ...
            for i in range(first_hit_position, len(return_state)):
                return_state[i] = return_state[
                    first_hit_position
                ]  # *(i-first_hit_position+1)
            # if it would continue till the end of the episode
            return_reward = -np.sqrt(
                np.mean(np.square(return_state - golden_trajectory * self.state_scale))
            )  # *\
...
if __name__ == "__main__":
    ...
    if False:
        ...
        print("rhobeg: ", rhobeg)
        res = opt.fmin_cobyla(
            objective, start_vector, [constr], rhobeg=rhobeg, rhoend=0.05
        )
        print(res)
    ...
    if False:
        ...
        pbounds = dict(
            [
                (
                    "x" + str(i),
                    (
                        environment_instance.action_space.low[0],
                        environment_instance.action_space.high[0],
                    ),
                )
                for i in range(1, 12)
            ]
        ...
        def black_box_function(x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11):
            func_val = -1 * objective(
                np.array([x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11])
            )
            return func_val
        ...
        optimizer.maximize(init_points=25, n_iter=50, acq="ucb", kappa=0.1)
        objective(
            np.array([optimizer.max["params"][x] for x in optimizer.max["params"]])
        )
